...
def test_process_handoff_request_artifact_exports_deterministic_ack(tmp_path: Path) -> None:
    run_id = "theme_ctx_strong.analysis_success_export"
    artifacts_root = tmp_path / "operator_runs"
    artifacts_root.mkdir(parents=True, exist_ok=True)
    ...
    expected_ack_path = tmp_path / "exports" / f"{run_id}.execution_boundary_ack.json"
    assert result.accepted is True
    assert result.ack_artifact_path == str(expected_ack_path)
    assert result.reject_artifact_path is None
    assert expected_ack_path.is_file()
    ...
    assert ack_payload["run_id"] == run_id
    assert ack_payload["reason_codes"] == []


def test_process_handoff_request_artifact_exports_deterministic_reject(tmp_path: Path) -> None:
    run_id = "theme_ctx_strong.analysis_success_export"
    payload = _load_approved_handoff_fixture()
    payload["handoff_payload"]["operator_decision"] = "reject"
    ...
    expected_reject_path = tmp_path / "exports" / f"{run_id}.execution_boundary_reject.json"
    assert result.accepted is False
    assert result.ack_artifact_path is None
    assert result.reject_artifact_path == str(expected_reject_path)
    assert expected_reject_path.is_file()
    ...
    assert (
        reject_payload["validation_error"].startswith("execution_boundary_contract_invalid:")
    )
