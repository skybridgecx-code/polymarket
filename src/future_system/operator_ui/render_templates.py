    ...
    *,
    ...
    if issue_rows:
        issues_block = (
            ...
            "<table><caption>Run file issues</caption><thead><tr>"
            "<th>Run</th><th>Issue</th><th>Message</th><th>JSON Path</th>"
    ...
    return (
        ...
        "aria-describedby=\"context_source_help\" "
        ...
        f"<select id=\"analyst_mode\" name=\"analyst_mode\" "
        ...
        f"{error_block}"
        "<h2>Runs</h2>"
        "<table><caption>Local review runs</caption><thead><tr>"
...
def _render_operator_review_edit_form_section(
    *,
    ...
    return (
        ...
        "aria-describedby=\"review_status_help\">"
        f"<option value=\"pending\"{pending_selected}>pending</option>"
        f"<option value=\"decided\"{decided_selected}>decided</option>"
        "</select>"
        "<p id=\"review_status_help\" class=\"help\">"
        "Pending keeps this run in review. Decided requires a decision.</p></div>"
        "<div class=\"form-field\"><label for=\"operator_decision\">Decision</label>"
        "<select id=\"operator_decision\" name=\"operator_decision\" "
        "aria-describedby=\"operator_decision_help\">"
        f"{decision_options_html}</select>"
        "<p id=\"operator_decision_help\" class=\"help\">"
        "Choose a decision only when status is decided. "
        ...
        "aria-describedby=\"review_notes_summary_help\">"
        f"{html.escape(notes_value)}</textarea>"
        "<p id=\"review_notes_summary_help\" class=\"help\">"
        "Optional short note for the local operator review.</p></div>"
        ...
        "aria-describedby=\"reviewer_identity_help\" "
        "aria-describedby=\"reviewer_identity_help\" "
        f"value=\"{html.escape(reviewer_value)}\">"
        "<p id=\"reviewer_identity_help\" class=\"help\">"
        "Optional local reviewer name or handle.</p></div>"
